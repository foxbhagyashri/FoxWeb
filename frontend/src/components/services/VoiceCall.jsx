...
function VoiceCall() {
  ...
  const features = [
    { icon: "📞", title: "Automated Voice Calls", desc: "Reach thousands of customers with automated voice broadcasting" },
    { icon: "🎙️", title: "IVR Solutions", desc: "Interactive Voice Response systems for seamless customer interactions" },
    { icon: "📊", title: "Call Analytics", desc: "Detailed reports on call duration, response rates, and conversions" },
    { icon: "🔊", title: "Text-to-Speech", desc: "Convert your messages to natural-sounding voice calls instantly" },
    { icon: "📅", title: "Scheduled Campaigns", desc: "Plan and schedule voice campaigns for optimal reach" },
    { icon: "🌐", title: "Multi-Language", desc: "Reach diverse audiences with multi-language voice capabilities" }
  ];

  const expertise = [
    { title: "Promotional Calls", desc: "Marketing campaigns via voice broadcasting", icon: "📢" },
    { title: "Reminder Calls", desc: "Appointment and payment reminders", icon: "⏰" },
    { title: "Survey Calls", desc: "Automated customer feedback collection", icon: "📋" },
    { title: "Alert Systems", desc: "Emergency and notification alerts", icon: "🚨" }
  ];
  ...
  return (
    <motion.div ref={containerRef} className="overflow-hidden" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
      ...
      <div className="relative min-h-screen overflow-hidden bg-gray-900">
        ...
        <div className="absolute bottom-20 right-10 w-40 h-40 rounded-full bg-orange-500/20 blur-3xl float-animation" style={{ animationDelay: "2s" }} />

        <div className="relative z-10 min-h-screen flex items-center justify-center px-4">
          <div className="text-center" style={{ transform: `translateY(${scrollY * 0.1}px)` }}>
            <motion.span className="inline-block mb-4 px-6 py-2 rounded-full text-sm font-semibold bg-red-500/20 text-red-300 backdrop-blur-sm border border-red-500/30" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
              Communication Solutions
            </motion.span>
            <motion.h1 className="text-4xl md:text-6xl lg:text-7xl font-bold mb-6 bg-gradient-to-r from-red-400 via-orange-400 to-red-400 bg-clip-text text-transparent" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.6 }}>
              Voice Call Services
            </motion.h1>
            <motion.p className="text-lg md:text-xl max-w-3xl mx-auto mb-10 text-white/80" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
              Powerful voice broadcasting and IVR solutions to connect with your audience effectively
            </motion.p>
            <motion.div className="flex flex-col sm:flex-row gap-4 justify-center" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
              <Link to="/contact" className="px-8 py-3 bg-gradient-to-r from-red-500 to-orange-500 text-white rounded-full font-semibold hover:scale-105 transition-transform shadow-lg shadow-red-500/30">Get Started</Link>
      ...
      <section className="theme-bg-primary py-20 px-4">
        <div className="container mx-auto max-w-7xl">
          <div className="text-center mb-16">
            ...
            <motion.h2 className="text-3xl md:text-5xl font-bold mb-6 bg-gradient-to-r from-red-500 to-orange-500 bg-clip-text text-transparent" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}>Voice Call Features</motion.h2>
          </div>
          <div className="grid md:grid-cols-3 gap-8">
            {features.map((item, index) => (
              <motion.div key={index} className="p-8 rounded-2xl theme-bg-card border theme-border shadow-sm hover:shadow-xl transition-all duration-300 group" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }}>
                <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-red-500 to-orange-500 flex items-center justify-center mb-6 text-2xl group-hover:scale-110 transition-transform">{item.icon}</div>
      ...
      <div className="relative min-h-[60vh] overflow-hidden bg-gray-900">
        ...
        <div className="relative z-10 min-h-[60vh] flex items-center justify-center px-4">
          <div className="max-w-4xl mx-auto text-center">
            <motion.h2 className="text-3xl md:text-5xl font-bold mb-6 text-white" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}>Ready to Amplify Your Reach?</motion.h2>
            <motion.p className="text-lg mb-8 text-white/80 max-w-2xl mx-auto" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>Create powerful voice campaigns that connect</motion.p>
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
              <Link to="/contact" className="px-10 py-4 bg-gradient-to-r from-red-500 to-orange-500 text-white rounded-full font-semibold hover:scale-105 transition-transform shadow-xl">Contact Us Today</Link>
            </motion.div>
          </div>
        </div>
      </div>
      ...
      <section className="theme-bg-primary py-20 px-4">
        <div className="container mx-auto max-w-6xl">
          <div className="text-center mb-16">
            <motion.span className="inline-block mb-4 px-6 py-2 rounded-full text-base font-bold bg-red-500 text-white" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>Use Cases</motion.span>
            <motion.h2 className="text-3xl md:text-5xl font-bold bg-gradient-to-r from-red-500 to-orange-500 bg-clip-text text-transparent" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}>Voice Call Solutions</motion.h2>
          </div>
            ...
            {expertise.map((item, index) => (
              <motion.div key={index} className="p-8 rounded-2xl theme-bg-card border theme-border shadow-sm hover:shadow-xl transition-all duration-300" initial={{ opacity: 0, x: index % 2 === 0 ? -30 : 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }}>
                <div className="flex items-start gap-6">
                  <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-red-500 to-orange-500 flex items-center justify-center text-2xl flex-shrink-0">{item.icon}</div>
